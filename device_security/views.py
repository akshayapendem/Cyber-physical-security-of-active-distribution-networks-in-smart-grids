...
MODULE_TITLE = 'Device Security Module'
MODULE_DESC = "Cybersecurity of ADN devices and sensors including Phasor Measurement Units (PMUs), smart meters, advanced metering infrastructure and protection relays are discussed in detail."
...
def index(request):
    ...
    if latest_dataset:
        try:
            ...
            if len(numeric_cols) > 1:
                vals = df[numeric_cols[1]].dropna().head(6).tolist()
                if len(vals) > 0:
                    data_values = vals
    ...
    try:
        if latest_dataset:
            ...
            if len(temp_cols) > 1:
                col_name = f"uploaded dataset column '{temp_cols[1]}'"
    except:
        pass
    ...
    return render(request, 'device_security/index.html', context)
