...
MODULE_TITLE = 'Industry Solutions Module'
MODULE_DESC = "Potential and existing solutions by industry are highlighted."
...
def index(request):
    ...
    if latest_dataset:
        try:
            ...
            if len(numeric_cols) > 4:
                vals = df[numeric_cols[4]].dropna().head(6).tolist()
                if len(vals) > 0:
                    data_values = vals
    ...
    try:
        if latest_dataset:
            ...
            if len(temp_cols) > 4:
                col_name = f"uploaded dataset column '{temp_cols[4]}'"
    except:
        pass
    ...
    return render(request, 'industry_solutions/index.html', context)
