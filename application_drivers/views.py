...
MODULE_TITLE = 'Application Drivers & Enablers Module'
MODULE_DESC = "A thorough study of ADNs application drivers and enablers including microgrids, Electric Vehicles (EVs), Internet-of-Things (IoT) and smart homes is conducted."
...
def index(request):
    ...
    if latest_dataset:
        try:
            ...
            if len(numeric_cols) > 3:
                vals = df[numeric_cols[3]].dropna().head(6).tolist()
                if len(vals) > 0:
                    data_values = vals
    ...
    try:
        if latest_dataset:
            ...
            if len(temp_cols) > 3:
                col_name = f"uploaded dataset column '{temp_cols[3]}'"
    except:
        pass
    ...
    return render(request, 'application_drivers/index.html', context)
