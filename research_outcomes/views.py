...
MODULE_TITLE = 'Research Outcomes & Future Directions Module'
MODULE_DESC = "Survey outcomes and directions for future work are presented to highlight emerging avenues of research."
...
def index(request):
    ...
    if latest_dataset:
        try:
            ...
            if len(numeric_cols) > 5:
                vals = df[numeric_cols[5]].dropna().head(6).tolist()
                if len(vals) > 0:
                    data_values = vals
    ...
    try:
        if latest_dataset:
            ...
            if len(temp_cols) > 5:
                col_name = f"uploaded dataset column '{temp_cols[5]}'"
    except:
        pass
    ...
    return render(request, 'research_outcomes/index.html', context)
